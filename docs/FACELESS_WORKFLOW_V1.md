...
- no production project has yet persisted real canonical image assets; the paths in `faceless/projects/demo` are contract fixtures only,
- no real style frame has been generated by the V1 engine and materialized into a production project,
...
The next implementation step is Codex-first: create a real production project, materialize the user-approved canonical style/character assets into that project, validate its `continuity.json`, and only then generate the next scene from those canonical references. RunPod/Higgsfield can be attached later as execution backends without changing the locked production workflow.
