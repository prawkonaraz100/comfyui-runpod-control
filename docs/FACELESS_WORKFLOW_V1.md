...
- the RunPod capability preflight has **not completed**; the current preflight run is still queued,
...
The next implementation step is to obtain a completed RunPod capability report and then connect the style-frame request contract to a real image-generation job without changing the target workflow below.
